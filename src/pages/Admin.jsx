  ...
  if (user?.role !== "admin") {
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <p className="font-body text-muted-foreground">Accès réservé aux administrateurs.</p>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <div className="bg-card border-b border-border/50 px-6 py-4 flex items-center gap-4">
        ...
        <div>
          ...
          <p className="font-body text-xs text-muted-foreground">Tableau de bord</p>
        </div>
      ...
      <div className="max-w-5xl mx-auto px-6 py-10">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
          {[
            { label: "Total RDV", value: bookings.length, icon: Calendar, color: "text-primary" },
            { label: "Confirmés", value: confirmed.length, icon: CheckCircle2, color: "text-green-500" },
            { label: "Annulés", value: cancelled.length, icon: XCircle, color: "text-red-400" },
            { label: "Abonnés", value: subscribers.length, icon: Mail, color: "text-blue-400" },
        ...
        <div className="bg-primary/5 border border-primary/20 rounded-2xl p-5 mb-8 flex items-start gap-4">
          ...
          <div className="flex-1">
            <p className="font-body text-sm font-medium text-foreground mb-1">Configuration du webhook Calendly</p>
            <p className="font-body text-xs text-muted-foreground mb-3">
              Pour recevoir les notifications automatiquement, allez dans <strong>Dashboard → Code → Functions → calendlyWebhook</strong>, copiez l'URL de l'endpoint, puis ajoutez-la dans <strong>Calendly → Intégrations → Webhooks</strong> avec les événements <em>invitee.created</em> et <em>invitee.canceled</em>.
            </p>
            <a
            ...
            >
              Ouvrir Calendly Webhooks <ExternalLink className="h-3 w-3" />
            </a>
          </div>
        </div>
        ...
        <div className="flex gap-2 mb-6">
          {["bookings", "subscribers"].map(t => (
            ...
            >
              {t === "bookings" ? `Rendez-vous (${bookings.length})` : `Abonnés newsletter (${subscribers.length})`}
            </button>
          ))}
        </div>
          ...
          <div className="bg-card border border-border/50 rounded-2xl overflow-hidden">
            {bookings.length === 0 ? (
              <div className="py-16 text-center">
                ...
                <p className="font-body text-sm text-muted-foreground">Aucun rendez-vous reçu pour l'instant.</p>
                <p className="font-body text-xs text-muted-foreground mt-1">Configurez le webhook Calendly ci-dessus.</p>
              </div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead className="border-b border-border/50">
                    <tr>
                      {["Client", "Email", "Service", "Date & heure", "Statut"].map(h => (
          ...
          <div className="bg-card border border-border/50 rounded-2xl overflow-hidden">
            {subscribers.length === 0 ? (
              <div className="py-16 text-center">
                ...
                <p className="font-body text-sm text-muted-foreground">Aucun abonné pour l'instant.</p>
              </div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full">
